...
RegisterServerCallback("sf-txh:getPlayerCards", function(source)
	if(TxhSources[source]) then
		local txhId = TxhSources[source].id
		local game = TxhGames[txhId]
		if(not game) then return end
		return game.playerHands[source].cards
	end
end)
...
RegisterServerCallback('sf-txh:canUse', function(source, txhId, seat, modelId)
	if(TxhGames[txhId] and not TxhGames[txhId].seats[seat]) then
		...
		if(TxhSources[tonumber(source)]) then return false end
		if(not canUse) then
		...
		if(txhGame.society ~= "none" and Config.Society.enable and Config.Society.entryFee > 0) then
			if(Bridge.GetItemCount(source, (txhGame.moneyItem or Config.MoneyItem)) < (txh.smallBlind*2 + Config.Society.entryFee)) then
				ShowNotification(source, _L("not_enough_money"))
				return
			end
			Bridge.RemoveItem(source, (txhGame.moneyItem or Config.MoneyItem), Config.Society.entryFee)
			Bridge.AddSocietyMoney(txhGame.society, Config.Society.entryFee)
		end
