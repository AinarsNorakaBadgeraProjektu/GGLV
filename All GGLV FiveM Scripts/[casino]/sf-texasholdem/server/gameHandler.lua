function CreateGame(id)
    ...
    self.startGame = function(smallBlind)
        ...
        for seat, source in pairs(self.perSeats) do
            if source and not self.playerHands[source] then
        ...
        self.started = true;
        self.dealerIndex = wrap(self.dealerIndex + 1, #self.activePlayers);
        ...
        for i = 1,2 do
            ...
            if(Bridge.GetItemCount(blindSources[i], self.moneyItem) < betAmount or not Bridge.RemoveItem(blindSources[i], self.moneyItem, betAmount)) then
        ...
        local fakeDeck = CreateDeck(1, 0)
        for i=1, 8 do
            local source = self.perSeats[i];
            if(not source) then
                goto nextSeat
            end
            ...
            if hand then
                ...
                for j = 1, 2 do
                    ...
                    hand.addCard(card);
                    local fakeCard = fakeDeck.pullCard()
                    table.insert(fakeCards, fakeCard)
                end
                TriggerClientEvent("sf-txh:dealerAnims", -1, self.id, { "deck_deal_p02" }, nil, i)
                TriggerClientEvent("sf-txh:spawnCards", -1, self.id, source, i, fakeCards, "deck_deal_p02")
                Citizen.Wait(2000)
        ...
        repeat
            ...
            if action == "fold" or action == "fold_reveal" then
                ...
                if hand.revealed then
                    Citizen.Wait(1000)
                    TriggerClientEvent("sf-txh:playCardsAnim", -1, self.id, source, nil, "reveal_folded_p02", "anim_casino_b@amb@casino@games@threecardpoker@dealer", false, self.activePlayers[index], hand.cards)
    ...
    self.getPlayerInvestments = function()
        ...
        for playerIndex, seatIndex in pairs(self.activePlayers) do
            ...
            local handBet = hand.bet;
            if self.society ~= "none" then
                handBet = (1 - Config.Society.potPercent) * handBet;
            end
    ...
    self.finish = function()
        ...
        if self.moreThanOnePlaying() then
            for playerIndex, seatIndex in pairs(self.activePlayers) do
                ...
                if not hand.folded then
                    hand.revealed = true;
                    TriggerClientEvent("sf-txh:playCardsAnim", -1, self.id, source, nil, "reveal_played_p02", "anim_casino_b@amb@casino@games@threecardpoker@dealer", false, seatIndex, hand.cards)
        ...
        local societyPot = self.board.bet;
        for k,v in pairs(winners) do
            if v > 0 then
                ...
                if(not self.removePlayers[tonumber(source)]) then
                    ...
                    societyPot = societyPot - v;
                end
            end
        end

        if societyPot > 0 and self.society ~= "none" then
            Bridge.AddSocietyMoney(self.society, societyPot)
        end
        ...
        for playerIndex, seatIndex in pairs(self.activePlayers) do
            ...
            local mul = 1.0;
            if self.society ~= "none" then
                mul = 1.0 - Config.Society.potPercent;
            end

            local winAmount = ((winners[playerIndex] or 0) - hand.bet) / mul;
        ...
        for playerIndex, seatIndex in pairs(self.activePlayers) do
            ...
            if hand.revealed then
                TriggerClientEvent("sf-txh:playCardsAnim", -1, self.id, source, nil, "cards_collect_p02", "anim_casino_b@amb@casino@games@threecardpoker@dealer", false, seatIndex, hand.cards)
