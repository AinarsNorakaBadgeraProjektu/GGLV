...
window.addEventListener("message", async function (event) {
    ...
    if (item.price){
        ...
        if (item.update != true){
            ...
            if (config.enable_different_fuels) {
                $("#navigation-tab-fuel-types").append(getFuelTypesTabHTML());
            }
        ...
        let fuelTypesColumns = ["stock"];
        let fuel_types_amount = 1; // Regular, Plus, Premium, Diesel.
        if (config.enable_different_fuels) {
            fuel_types_amount = 4;
            fuelTypesColumns = ["stock", "stock_plus", "stock_premium", "stock_diesel"];
        }
        const total_stock = fuelTypesColumns.reduce((sum, type) => sum + gas_station_business[type], 0);
        $("#player-info-stock").text(" "+Utils.numberFormat(total_stock,0) + " " + Utils.translate("str_liters"));
		...
		*/
        ...
        let stock_capacity = (config.gas_station_types.stock_capacity + config.gas_station_types.upgrades.stock.level_reward[gas_station_business.stock_upgrade]) * fuel_types_amount;
        const stock_capacity_percent = fuelTypesColumns
            .reduce((sum, type) => sum + (gas_station_business[type] * 100) / stock_capacity, 0)
            .toFixed(1);
        ...
        if (gas_station_business.stock_electricfast == 0) {
            $("#unlock-fast-charger-container").css("display", "flex");
        } else {
            $("#unlock-fast-charger-container").css("display", "none");
        }
        if (gas_station_business.stock_electricnormal == 0) {
            $("#unlock-normal-charger-container").css("display", "flex");
        } else {
            $("#unlock-normal-charger-container").css("display", "none");
        }

        renderFuelTypeTab(gas_station_business, gas_station_users_theme.dark_theme);
		...
		*/
        ...
        for (const key in config.gas_station_types.ressuply) {
            let ressuply = config.gas_station_types.ressuply[key];
            let amount = (ressuply.liters + Math.floor(ressuply.liters*(config.gas_station_types.upgrades.truck.level_reward[gas_station_business.truck_upgrade]/100)));
							...
							<div class="card-body cleartfix">
								<div class="media align-items-stretch">
									...
									<div class="d-flex align-self-center align-items-center">
										...
										<button onclick="openDialogStartContract(1,` + key + `, ` + amount + `, ` + gas_station_business.relationship_upgrade + `)" class="btn btn-primary ml-3 py-2 export-button">` + Utils.translate("btn_import") + `</button>
										<button onclick="openDialogStartContract(2,` + key + `, ` + amount + `, ` + gas_station_business.relationship_upgrade + `)" class="btn btn-danger ml-3 py-2 export-button">` + Utils.translate("btn_export") + `</button>
									</div>
								</div>
							</div>
						</div>
		...
		*/
        ...
        for (const gas_station_job of gas_station_jobs) {
            let import_price = config.gas_station_types.ressuply_deliveryman.price_per_liter - (config.gas_station_types.ressuply_deliveryman.price_per_liter * config.gas_station_types.upgrades.relationship.level_reward[gas_station_business.relationship_upgrade])/100;
            let total_cost = gas_station_job.reward + import_price*gas_station_job.amount;
					...
					<div class="d-flex flex-row align-items-center"><img class="font-large-2 mr-2 " src="img/combustivel.png" width="30">
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 small">
								...
								<div class="ml-3"><i class="fas fa-boxes-stacked"></i><span class="ml-2">` + Utils.translate("str_amount") + `: ` + gas_station_job.amount + `</span></div>
								<div class="ml-3"><i class="fas fa-gas-pump"></i><span class="ml-2">` + Utils.translate("str_fuel_type") + `: ` + Utils.translate(`fuel_types.${gas_station_job.fuel_type}`) + `</span></div>
							</div>
						</div>
					</div>
        ...
        $("#upgrades-page").append(`
			<div class="col-lg-4 col-md-6 mb-4 mb-lg-0">
				<div class="card shadow-sm border-0">
					<div class="card-body card-theme rounded p-4"><img src="img/upgrades/shop.png" alt="" class="img-fluid d-block mx-auto mb-3">
			...
			<div class="col-lg-4 col-md-6 mb-4 mb-lg-0">
				<div class="card shadow-sm border-0">
					<div class="card-body card-theme rounded p-4"><img src="img/upgrades/delivery-truck.png" alt="" class="img-fluid d-block mx-auto mb-3">
			...
			<div class="col-lg-4 col-md-6 mb-4 mb-lg-0">
				<div class="card shadow-sm border-0">
					<div class="card-body card-theme rounded p-4"><img src="img/upgrades/manager.png" alt="" class="img-fluid d-block mx-auto mb-3">
		...
		*/
        ...
        if (item.update != true){
			...
			*/
            ...
            $("#unlock-chargers-title").text(Utils.translate("str_goods_page_unlock_electric_title"));
            $("#unlock-chargers-title-2").text(Utils.translate("str_goods_page_unlock_electric_title"));
            $("#unlock-chargers-button").text(Utils.translate("str_goods_page_unlock_electric_button").format(Utils.currencyFormat(config.gas_station_types.electric_normal_price, 0)));
            $("#unlock-chargers-button-2").text(Utils.translate("str_goods_page_unlock_electric_button").format(Utils.currencyFormat(config.gas_station_types.electric_fast_price, 0)));
            ...
            $("#hire-page-form").append(`
				...
				<div class="col-2">
					<select id="form_fuel_type" name="fuel_type" class="form-control pl-1 h-100" required="required">
						<option value="regular">${Utils.translate("fuel_types.regular")}</option>
						<option value="plus">${Utils.translate("fuel_types.plus")}</option>
						<option value="premium">${Utils.translate("fuel_types.premium")}</option>
						<option value="diesel">${Utils.translate("fuel_types.diesel")}</option>
					</select> 
				</div>
				<div class="col-2"> 
    ...
    if (item.hidemenu){
        $(".main").fadeOut(200);
        // Dispose of all FluidMeter instances
        document.querySelectorAll("[id^=fluid-meter-]").forEach((element) => {
            let fluidMeterInstance = element.fluidMeterInstance;
            if (fluidMeterInstance) {
                fluidMeterInstance.dispose();
                element.fluidMeterInstance = null;
            }
        });
    }
});

function renderFuelTypeTab(gas_station_business, dark_theme) {
    let fuel_type_list = {
        regular: {
            stock: "stock",
            price: "price",
            colors: { foreground: "gold", background: "yellow" },
        },
        plus: {
            stock: "stock_plus",
            price: "price_plus",
            colors: { foreground: "#ffcc00", background: "#ffdd44" },
        },
        premium: {
            stock: "stock_premium",
            price: "price_premium",
            colors: { foreground: "#ff9900", background: "#ffaa33" },
        },
        diesel: {
            stock: "stock_diesel",
            price: "price_diesel",
            colors: { foreground: "#007722", background: "#009933" },
        },
    };

    let stock_capacity = (config.gas_station_types.stock_capacity + config.gas_station_types.upgrades.stock.level_reward[gas_station_business.stock_upgrade]);

    // Loop through fuel types to create a tab for each
    Object.keys(fuel_type_list).forEach(fuelType => {
        let stock_key = fuel_type_list[fuelType].stock;
        let price_key = fuel_type_list[fuelType].price;
        let stock_capacity_percent = ((gas_station_business[stock_key] * 100)/stock_capacity);

        $(`#fuel-${fuelType}`).empty();
        $(`#fuel-${fuelType}`).append(`
			<div class="col-md-6">
				<div id="fluid-meter-${fuelType}"></div>
			</div>
			<div class="control-box" id="control-box-${fuelType}">
				<div class="slider-content">
					<div class="icon"><i class="fas fa-wallet"></i></div>
					<div class="text">` + Utils.translate("str_gas_price") + `</div>
					<div class="value" id="value-${fuelType}">` + gas_station_business[price_key] + `</div>
					<button onclick="applyPrice('${fuelType}', '${price_key}')" class="btn btn-primary mt-5 pl-5 pr-5">
						<i class="fas fa-check"></i>&nbsp;&nbsp;` + Utils.translate("btn_apply_price") + `
					</button>
				</div>
				<div class="slider-container">
					<span class="bar"><span class="fill" id="fill-${fuelType}"></span></span>
					<input type="range" id="slider-${fuelType}" class="slider" 
						min="` + (config.gas_station_types.min_gas_price * 100) + `" 
						max="` + (config.gas_station_types.max_gas_price * 100) + `" 
						value="` + gas_station_business[price_key] + `">
				</div>
			</div>
		`);

        let fluid_element = document.getElementById(`fluid-meter-${fuelType}`);
        if (fluid_element) {
            // Fluid meter
            let fm3 = new FluidMeter();
            fm3.init({
                targetContainer: fluid_element,
                fillPercentage: stock_capacity_percent,
                options: {
                    fontSize: "70px",
                    fontFamily: "Montserrat",
                    bubbleAmount: stock_capacity_percent,
                    drawPercentageSign: true,
                    drawBubbles: true,
                    size: 550,
                    borderWidth: 19,
                    backgroundColor: dark_theme ? "#2f2f2f" : "#e2e2e2",
                    foregroundColor: dark_theme ? "#5a5a5a" : "#fafafa",
                    fontFillStyle: dark_theme ? "#e1e1e1" : "white",
                    foregroundFluidLayer: {
                        fillStyle: fuel_type_list[fuelType].colors.foreground,
                        angularSpeed: 30,
                        maxAmplitude: 15,
                        frequency: 70,
                        horizontalSpeed: -40,
                    ...
                    backgroundFluidLayer: {
                        fillStyle: fuel_type_list[fuelType].colors.background,
                        angularSpeed: 100,
                        maxAmplitude: 4,
                        frequency: 100,
                        horizontalSpeed: 100,
            ...
            fluid_element.fluidMeterInstance = fm3;
        }

        // Input range
        let cbSlider = document.getElementById(`slider-${fuelType}`);
        let cbFill = document.getElementById(`fill-${fuelType}`);
        let cbValue = document.getElementById(`value-${fuelType}`);
        let controlBox = document.getElementById(`control-box-${fuelType}`);

        if (cbSlider && cbFill && cbValue) {
            function setBar() {
                let min = parseInt(cbSlider.getAttribute("min"));
                let max = parseInt(cbSlider.getAttribute("max"));
                let value = parseInt(cbSlider.value);
    ...
    let electric_fuel_type_list = {
        electricnormal: {
            price: "price_electricnormal",
            image: "electricnormal",
        },
        electricfast: {
            price: "price_electricfast",
            image: "electricfast",
        },
    };

    Object.keys(electric_fuel_type_list).forEach(fuelType => {
        let price_key = electric_fuel_type_list[fuelType].price;
        let image = electric_fuel_type_list[fuelType].image;

        // Render electric tab
        $(`#fuel-${fuelType}`).empty();
        $(`#fuel-${fuelType}`).append(`
			<div class="col-md-6 d-flex align-items-center justify-content-center">
				<img class="" src="img/${image}.png" width="400">
			</div>
			<div class="control-box" id="control-box-${fuelType}">
				<div class="slider-content">
					<div class="icon"><i class="fas fa-wallet"></i></div>
					<div class="text">` + Utils.translate("str_electric_price") + `</div>
					<div class="value" id="value-${fuelType}">` + gas_station_business[price_key] + `</div>
					<button onclick="applyPrice('${fuelType}', '${price_key}')" class="btn btn-primary mt-5 pl-5 pr-5">
						<i class="fas fa-check"></i>&nbsp;&nbsp;` + Utils.translate("btn_apply_price") + `
					</button>
...
function getFuelTypesTabHTML() {
    return getTabHTML("goods", "fuel-regular", Utils.translate("str_goods_page_fuel_regular"), true)
		+ getTabHTML("goods", "fuel-plus", Utils.translate("str_goods_page_fuel_plus"))
		+ getTabHTML("goods", "fuel-premium", Utils.translate("str_goods_page_fuel_premium"))
		+ getTabHTML("goods", "fuel-diesel", Utils.translate("str_goods_page_fuel_diesel"))
		+ getTabHTML("goods", "fuel-electricnormal", Utils.translate("str_goods_page_fuel_electricnormal"))
		+ getTabHTML("goods", "fuel-electricfast", Utils.translate("str_goods_page_fuel_electricfast"));
}
...
function openDialogStartContract(type,ressuply_id,amount,relationship_upgrade) {
    if (!config.enable_different_fuels) {
        startContract(type, ressuply_id, "regular");
        return;
    }
    let gas_station_ressuply;
    let title;
    if (type == 1) {
        title = Utils.translate("str_jobs_page_fuel_title_import");
        gas_station_ressuply = config.gas_station_types.ressuply[ressuply_id].price_per_liter_to_import;
    } else {
        title = Utils.translate("str_jobs_page_fuel_title_export");
        gas_station_ressuply = config.gas_station_types.ressuply[ressuply_id].price_per_liter_to_export;
    }

    const discountPercent = config.gas_station_types.upgrades.relationship.level_reward[relationship_upgrade] / 100;
    const fuelTypes = ["regular", "plus", "premium", "diesel"];
    const finalPrices = {};
    for (const type of fuelTypes) {
        const basePrice = gas_station_ressuply[type] * amount;
        const discount = Math.floor(basePrice * discountPercent);
        finalPrices[type] = basePrice - discount;
    }

    const exampleConfig = {
        title: title,
        // body: "Custom Modal Body Text",
        bodyHtml: `<div class="d-flex justify-content-center m-3"><img src="img/${config.gas_station_types.ressuply[ressuply_id].img}" width="120"></div>`,
        buttons: [
        ...
        inputs: [
            {
                ...
                label: Utils.translate("str_jobs_page_fuel_desc"),
                id: "select-input-id",
                name: "select-input-name",
                required: true,
                options: [
                    { value: "regular", text: Utils.translate("str_jobs_page_fuel_option_price").format(Utils.translate("fuel_types.regular"), Utils.currencyFormat(finalPrices.regular, 2)) },
                    { value: "plus", text: Utils.translate("str_jobs_page_fuel_option_price").format(Utils.translate("fuel_types.plus"), Utils.currencyFormat(finalPrices.plus, 2)) },
                    { value: "premium", text: Utils.translate("str_jobs_page_fuel_option_price").format(Utils.translate("fuel_types.premium"), Utils.currencyFormat(finalPrices.premium, 2)) },
                    { value: "diesel", text: Utils.translate("str_jobs_page_fuel_option_price").format(Utils.translate("fuel_types.diesel"), Utils.currencyFormat(finalPrices.diesel, 2)) },
                ],
            },
        ],
        onSubmit: function(formData) {
            let fuel_type = formData.get("select-input-name"); // Get by element name
            startContract(type, ressuply_id, fuel_type);
        },
    };
    Utils.showCustomModal(exampleConfig);
}
...
function startContract(type,ressuply_id,fuel_type){
    Utils.post("startContract",{ type:type,ressuply_id:ressuply_id,fuel_type:fuel_type });
}
...
$(document).ready( function() {
    ...
    $("#contact-form").on("submit", function(e){
        ...
        Utils.post("createJob",{ name:form[0].value,reward:form[1].value,amount:form[2].value,fuel_type:form[3].value });
    });
...
function unlockElectric(type){
    Utils.post("unlockElectric",{ type });
}
function applyPrice(fuelType, column){
    let cbSlider = document.getElementById(`slider-${fuelType}`);
    let value = parseInt(cbSlider.value);
    Utils.post("applyPrice",{ value: value, column: column });
}
function hirePlayer() {
...
function FluidMeter() {
    ...
    let bubblesLayer = {
        ...
        reset: function (bubble) {
            ...
            let meterBottom = (options.size - (options.size - getMeterRadius()) / 2) - options.borderWidth;
            let fluidAmount = currentFillPercentage * (getMeterRadius() - options.borderWidth * 2) / 100;
        ...
        init() {
            for (let i = 0; i < options.bubbleAmount; i++) {
 ...
 */
    function drawFluidLayer(layer, dt) {
        ...
        let x = 0;
        let y;
        let amplitude = layer.maxAmplitude * Math.sin(layer.angle * Math.PI / 180);
 ...
 */
    ...
    return {
        ...
        dispose: function () {
            ...
            if (targetContainer) {
                targetContainer.innerHTML = ""; // Clears the canvas
            }
        },
    };
};