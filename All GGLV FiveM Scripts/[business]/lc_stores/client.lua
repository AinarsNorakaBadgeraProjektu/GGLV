				...
				for _, pos in pairs(npc_data.pos) do
					...
					if distance < 50 then
						if not IsEntityAPed(pos.entity) then
							pos.entity = Utils.Peds.spawnPedAtCoords(npc_data.model, pos[1], pos[2], pos[3], pos[4], true,
								true, npc_data.emote)
						end
		...
		Citizen.CreateThreadNow(function()
			...
			while true do
				...
				if not menu_active then
					...
					if distance < 10 then
						timer = 2
						Utils.Markers.createMarkerInCoords(market_id, x, y, z, cached_translations.open,
							openOwnerUiCallback, nil, distance)
					end
				end
				Wait(timer)
			end
			...
			Citizen.CreateThreadNow(function()
				...
				while true do
					...
					if not menu_active then
						...
						if distance < 10 then
							timer = 2
							Utils.Markers.createMarkerInCoords(market_id, x, y, z, cached_translations.open_market,
								openCustomerUiCallback, nil, distance)
						end
					end
					Wait(timer)
				end
...
function createTargetsThread()
	Citizen.CreateThreadNow(function()
		for market_id, market_data in pairs(Config.market_locations) do
			local x, y, z = table.unpack(market_data.coord)
			Utils.Target.createTargetInCoords(market_id, x, y, z, openOwnerUiCallback, cached_translations.open_target,
				"fas fa-shop", "#2986cc")

			for customer_blip_id, customer_blip_location in pairs(market_data.sell_blip_coords) do
				local x, y, z = table.unpack(customer_blip_location)
				Utils.Target.createTargetInCoords(market_id, x, y, z, openCustomerUiCallback,
					cached_translations.open_market_target, "fas fa-shopping-cart", "#2986cc",
					market_id .. ":" .. customer_blip_id)
			end
		end
...
function renderDeliverymanJobBlip(market_id, x, y, z, distance)
	...
	if distance < 1.0 then
		if job_data[market_id] == nil then
			...
			Utils.Markers.drawText3D(x, y, z - 0.6,
				cached_translations.show_jobs:format(job_data[market_id].name, job_data[market_id].reward))
			if IsControlJustPressed(0, 38) then
...
AddEventHandler('stores:startJob', function(truck_level, is_import)
	...
	local route_blip = Utils.Blips.createBlipForCoords(destination.x, destination.y, destination.z, Config.route_blip.id,
		Config.route_blip.color, Utils.translate('blip_route'), 0.8, true)
	...
	local truck_vehicle, truck_blip = Utils.Vehicles.spawnVehicle(truck_model, garage_coord.x, garage_coord.y,
		garage_coord.z, garage_coord.w, blip_data, properties)
	local plate = GetVehicleNumberPlateText(truck_vehicle)
	...
	while IsEntityAVehicle(truck_vehicle) do
		...
		if delivery_phase == 1 then
			if is_import then
				...
				if distance <= 50 then
					...
					if distance <= 2 then
						Utils.Markers.drawText3D(destination.x, destination.y, destination.z - 0.6,
							Utils.translate('objective_marker'))
						if IsControlJustPressed(0, 38) then
							if not (IsPedSittingInAnyVehicle(ped) or IsPedInAnyVehicle(ped, true)) then
								object = createObjectAttachedToEntity("anim@heists@box_carry@", "idle",
				...
				if current_vehicle == truck_vehicle and distance <= 50 then
					...
					Utils.Markers.drawMarker(39, destination.x, destination.y, destination.z, 1.0)
					if distance <= 2 then
						Utils.Markers.drawText2D(Utils.translate('objective_marker_3'), 8, 0.5, 0.95, 0.50, 255, 255, 255,
							235)
						if IsControlJustPressed(0, 38) then
							...
							Citizen.Wait(500)
							Utils.Blips.removeBlip(route_blip)
							route_blip = Utils.Blips.createBlipForCoords(garage_coord.x, garage_coord.y, garage_coord.z,
								Config.route_blip.id, Config.route_blip.color, Utils.translate('blip_route'), 0.8, true)
							delivery_phase = 3
							PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", false)
							Citizen.Wait(1000)
							DoScreenFadeIn(1000)
							Utils.Scaleform.showScaleform(Utils.translate('sucess_2'),
								Utils.translate('sucess_in_progess_2'), 3)
						end
					end
				end
			end
		elseif delivery_phase == 2 then
			if is_import then
				local xa, ya, za = table.unpack(GetWorldPositionOfEntityBone(truck_vehicle,
					GetEntityBoneIndexByName(truck_vehicle, "door_dside_r")))
				local xb, yb, zb = table.unpack(GetWorldPositionOfEntityBone(truck_vehicle,
					GetEntityBoneIndexByName(truck_vehicle, "door_pside_r")))
				...
				if distance <= 50 then
					...
					if distance <= 2.0 then
						Utils.Markers.drawText3D(vehicle_trunk.x, vehicle_trunk.y, vehicle_trunk.z + 1.0,
							Utils.translate('objective_marker_2'))
						if IsControlJustPressed(0, 38) then
							if not (IsPedSittingInAnyVehicle(ped) or IsPedInAnyVehicle(ped, true)) then
								deleteObject(object)
								route_blip = Utils.Blips.createBlipForCoords(garage_coord.x, garage_coord.y,
									garage_coord.z, Config.route_blip.id, Config.route_blip.color,
									Utils.translate('blip_route'), 0.8, true)
								delivery_phase = 3
			...
			if distance <= 50 and current_vehicle == truck_vehicle then
				...
				if distance <= 4 then
					...
					if is_import then
						...
						Utils.Scaleform.showScaleform(Utils.translate('sucess_2'), Utils.translate('sucess_finished_2'),
							3)
		...
		if v.map_blip_coord then
			...
			if blips then
				if blips_table[k] and blips_table[k].market_blip and blips_table[k].market_name and blips_table[k].market_color then
					market_blips[k] = Utils.Blips.createBlipForCoords(x, y, z, tonumber(blips_table[k].market_blip),
						tonumber(blips_table[k].market_color), blips_table[k].market_name, blips.scale)
				else
					market_blips[k] = Utils.Blips.createBlipForCoords(x, y, z, blips.id, blips.color, blips.name,
						blips.scale)
				end
...
AddEventHandler('stores:updateBlip', function(key, name, color, blip)
	if Config.market_locations[key].map_blip_coord then
		...
		if blips then
			market_blips[key] = Utils.Blips.createBlipForCoords(x, y, z, tonumber(blip), tonumber(color), name,
				blips.scale)
			if Config.group_map_blips then
