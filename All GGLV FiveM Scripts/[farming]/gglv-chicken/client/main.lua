...
function CatchingChicken()
	...
	while startCatching do
		...
		if veh and (dist < 3.0) then
			...
			ClearPedTasks(ped)
			DeleteEntity(prop)
			TriggerServerEvent("gglv-chicken:server:AddAliveChicken")
		end
	end
end
...
function processAliveChicken(position)
	...
	if Player then
		...
		if Player then
			...
			TriggerServerEvent("gglv-chicken:server:SlaughteredChicken", position)
			end, function()
...
function packChicken(position)
    ...
    if Player then
        QBCore.Functions.Progressbar("packing_chicken", Lang:t("process.packing"), Config.PackagedChicken[position]["processing_time"], false, true, {disableMovement = true,
                ...
                TriggerServerEvent("gglv-chicken:server:PackagedChicken", position)
                MBNotify(Lang:t("notify.title"), Lang:t("success.keep_going_or_sell"), "info")
                
                -- Unfreeze player after process is done
                FreezeEntityPosition(Player, false)
            end, function()
                -- Unfreeze player in case the progress is canceled
                FreezeEntityPosition(Player, false)
            end)
        ...
        -- Freeze player while animation is running
        FreezeEntityPosition(Player, true)
        ...
        -- Unfreeze player after tasks are done
        FreezeEntityPosition(Player, false)
    end
end
...
function SellingChicken()
    ...
    if ped then
        local progressCompleted = lib.progressBar({
            duration = Config.Selling["processing_time"],
            label = Lang:t("process.selling"),
            useWhileDead = false,
            canCancel = false,
            disable = {
                car = true,
            ...
            prop = {
                ...
                pos = vec3(x, y, z),
                rot = vec3(0.0, 0.0, GetEntityHeading(ped))
            }
        })
        if progressCompleted then
            ...
            TriggerServerEvent("gglv-chicken:server:SellingMoney")
        else
            ClearPedTasksImmediately(ped)
            if prop then
                DeleteEntity(prop)
...
Citizen.CreateThread(function()
    while true do
        ...
        local waitTime = 1000 -- Default to a low processing frequency

        -- Catching chicken
        local startCoords = Config.CatchAliveChicken["coords"]
        local distance = #(plyCoords - startCoords)
        if distance <= 10.0 then
            waitTime = 0 -- Increase processing frequency when near a point of interest
            DrawMarker(27, startCoords.x, startCoords.y, startCoords.z - 0.97, 0, 0, 0, 0, 0, 0, 0.90, 0.90, 0.90, 255, 255, 255, 200, 0, 0, 0, 0)
            if distance <= 2.5 then
                QBCore.Functions.DrawText3D(startCoords.x, startCoords.y, startCoords.z, Lang:t("action.get_started"))
                if distance <= 0.5 and IsControlJustPressed(0, Keys['E']) then
                    StartCatching()
                end
            end
        end
        ...
        -- Processing chicken
        for farm, config in pairs(Config.SlaughteredChicken) do
            local dist = #(plyCoords - config.coords)
            if dist <= 10.0 then
                DrawMarker(27, config.coords.x, config.coords.y, config.coords.z - 0.97, 0, 0, 0, 0, 0, 0, 0.90, 0.90, 0.90, 255, 255, 255, 200, 0, 0, 0, 0)
                if dist < 2.5 then
                    QBCore.Functions.DrawText3D(config.coords.x, config.coords.y, config.coords.z, Lang:t("action.cut_alive_chicken"))
                    if dist < 1 and IsControlJustPressed(0, Keys['E']) then
                        local hasItem = exports.ox_inventory:Search('count', 'alive_chicken') > 0
                        if hasItem then
                            processAliveChicken(farm)
                        else
                            TriggerEvent("ox_lib:notify", {
                                type = "error",
                                description = Lang:t("error.no_alive_chicken"),
                                duration = 5000,
                            })
                        end
                    end
        ...
        -- Packing chicken
        for spot, data in pairs(Config.PackagedChicken) do
            local packCoords = data["coords"]
            local dist = #(plyCoords - packCoords)
            if dist <= 10.0 then
                waitTime = 0
                DrawMarker(27, packCoords.x, packCoords.y, packCoords.z - 0.97, 0, 0, 0, 0, 0, 0, 0.90, 0.90, 0.90, 255, 255, 255, 200, 0, 0, 0, 0)
                if dist < 2.5 then
                    QBCore.Functions.DrawText3D(packCoords.x, packCoords.y, packCoords.z, Lang:t("action.pack_chicken"))
                    if dist < 1 and IsControlJustPressed(0, Keys['E']) then
                        local hasItem = exports.ox_inventory:Search('count', 'slaughtered_chicken') > 0
                        if hasItem then
                            packChicken(spot)
                        else
                            TriggerEvent("ox_lib:notify", {
                                type = "error",
                                description = Lang:t("error.no_slaughtered_chicken"),
                                duration = 5000,
                            })
                        end
                    end
        ...
        if sellDist < 10.0 then
            waitTime = 0
            DrawMarker(27, sellCoords.x, sellCoords.y, sellCoords.z - 0.96, 0, 0, 0, 0, 0, 0, 2.20, 2.20, 2.20, 255, 255, 255, 200, 0, 0, 0, 0)
            if sellDist <= 2.5 then
                QBCore.Functions.DrawText3D(sellCoords.x, sellCoords.y, sellCoords.z, Lang:t("action.sell_packing"))
                if sellDist <= 0.5 and IsControlJustPressed(0, Keys['E']) then
                    local hasItem = exports.ox_inventory:Search('count', 'packagedchicken') > 0
                    if hasItem then
                        SellingChicken()
                    else
                        TriggerEvent("ox_lib:notify", {
                            type = "error",
                            description = Lang:t("error.no_packaged_chicken"),
                            duration = 5000,
                        })
                    end
                end
