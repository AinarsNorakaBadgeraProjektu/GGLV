...
RegisterServerEvent('gglv-lumber:server:workvan', function(workVanPrice)
    local source = source
...
RegisterServerEvent('gglv-lumber:server:returnworkvan', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('gglv-lumber:server:addLog', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('gglv-lumber:server:removeLog', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('gglv-lumber:server:deliverypaper', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('gglv-lumber:server:sellinglog', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
        ...
        if exports.ox_inventory:RemoveItem(source, 'tr_log', 1) then
            ...
            if playerLogSales[source] >= Config.maxLogs then
                if exports.ox_inventory:RemoveItem(source, 'tr_deliverypaper', 1) then
                    TriggerClientEvent('gglv-lumber:client:resetTimmyTask', source)
                end
                playerLogSales[source] = 0
            end
...
RegisterServerEvent('gglv-lumber:server:choptree', function()
    local source = source
...
RegisterServerEvent('gglv-lumber:server:sellitem', function(args)
    local source = source
