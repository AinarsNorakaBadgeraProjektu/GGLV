...
-- Depo Vehicles
RegisterNetEvent('gglv-lumber:client:deliverytruck', function()
    ...
    if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 10.0) then
        ...
        exports["Renewed-Vehiclekeys"]:addKey(GetVehicleNumberPlateText(JobVehicle))
...
RegisterNetEvent('gglv-lumber:client:workvan', function()
    ...
    if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
        ...
        TriggerServerEvent('gglv-lumber:server:workvan', workVanPrice)
        ...
        SetNetworkIdCanMigrate(id, true)
        TaskWarpPedIntoVehicle(player, JobVehicle, -1)
        exports["Renewed-Vehiclekeys"]:addKey(vanPlate)
        DoScreenFadeIn(1500)
...
RegisterNetEvent('gglv-lumber:client:returnworkvan', function()
    ...
    if PlayerWorkVans[playerId] and PlayerWorkVans[playerId][plate] then
        ...
        TriggerServerEvent('gglv-lumber:server:returnworkvan')
...
RegisterNetEvent('gglv-lumber:client:returndeliverytruck', function()
    local playerPed = PlayerPedId()
    local playerData = QBCore.Functions.GetPlayerData()
    local citizenid = playerData.citizenid
...
RegisterNetEvent('gglv-lumber:client:starttask', function()
...
RegisterNetEvent('gglv-lumber:client:timmytask', function()
        ...
        options = {
            {
                ...
                close = true,
                action = function()
                    TriggerServerEvent('gglv-lumber:server:deliverypaper')
...
RegisterNetEvent('gglv-lumber:client:loginteract', function()
    local playerPed = PlayerPedId()
    ...
    if HasPlayerGotLog() then
        ...
        if Config.progress == "qbcore" then
            QBCore.Functions.Progressbar('Picking up Log', Lang.pickingLog, Config.progressTime.collectLog * 1000, false, true, {
                ...
                StartCarryingLog(playerPed)
                Wait(7)
                TriggerServerEvent('gglv-lumber:server:addLog')
            end, function()
                ClearPedTasks(PlayerPedId())
            end)
...
local function loadLogOntoTrailer(trailer, trailerBone, nextPositionID, playerPed)
    ...
    TriggerServerEvent('gglv-lumber:server:removeLog')
end
...
RegisterNetEvent('gglv-lumber:client:loadtrailer', function()
    local playerPed = PlayerPedId()
...
RegisterNetEvent('gglv-lumber:client:unloadtrailer', function()
    if not HasPlayerGotLog() then
        local playerPed = PlayerPedId()
        local nearbyTrailers = getNearbyTrailers(GetEntityCoords(playerPed), 10.0)
...
RegisterNetEvent('gglv-lumber:client:logselling', function()
    if HasPlayerGotLog() and timmyTaskStarted then
        if Config.progress == "qbcore" then
            QBCore.Functions.Progressbar('Selling Log', Lang.sellingLog, Config.progressTime.sellingLog * 1000, false, true, {
                ...
                TriggerServerEvent('gglv-lumber:server:sellinglog')
            end, function()
                NotifyPlayer(Lang.cancelledProgress, 'error')
            end)
        elseif Config.progress == "ox" then
...
RegisterNetEvent('gglv-lumber:client:resetTimmyTask', function()
...
RegisterNetEvent('gglv-lumber:client:choptreecheck', function()
    local playerPed = PlayerPedId()
...
-- Shop
RegisterNetEvent('gglv-lumber:client:contractorshop', function()
