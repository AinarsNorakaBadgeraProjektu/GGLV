...
Core.Functions.CreateCallback('GGLV-Recycle:GetCash', function(source, cb) cb(Core.Functions.GetPlayer(source).Functions.GetMoney("cash")) end)

RegisterServerEvent("GGLV-Recycle:DoorCharge", function()
	if Config.Inv == "ox" then TriggerEvent("GGLV-Recycle:server:toggleItem", false, "money", Config.PayAtDoor, src)
	else Core.Functions.GetPlayer(source).Functions.RemoveMoney("cash", Config.PayAtDoor) end
end)

RegisterServerEvent("GGLV-Recycle:TradeItems", function(data)
    local src = source
	local table = {}
	...
	TriggerEvent("GGLV-Recycle:server:toggleItem", false, "recyclablematerial", data.amount, src)
	Wait(1000)
	for i = 1, table.itemGive do
		TriggerEvent("GGLV-Recycle:server:toggleItem", true, Config.TradeTable[math.random(1, #Config.TradeTable)], math.random(table.Min, table.Max), src)
		Wait(100)
	end
end)

RegisterNetEvent("GGLV-Recycle:Selling:Mat", function(data)
    local src = source
	...
	if HasItem(src, data.item, amount) then
		TriggerEvent("GGLV-Recycle:server:toggleItem", false, data.item, amount, src)
	...
	TriggerClientEvent(src, "GGLV-Recycle:Selling:Menu", data)
end)
...
RegisterNetEvent('GGLV-Recycle:server:toggleItem', function(give, item, amount, newsrc)
    local src = newsrc or source
    local Player = Core.Functions.GetPlayer(src)
    local amount = amount or 1 -- Default amount to 1
    ...
    if give == 0 or give == false then
        ...
        if HasItem(src, item, amount) then
            ...
            if Config.Debug then
                print(string.format("^5Debug^7: Removed %d x '%s' from Player %d", amount, Core.Shared.Items[item].label, src))
            end
        else
            dupeWarn(src, item) -- Warn or penalize player for invalid removal attempts
        end
    else
        -- Adding Item
        if exports.ox_inventory:CanCarryItem(src, item, amount) then
            ...
            if Config.Debug then
                print(string.format("^5Debug^7: Gave %d x '%s' to Player %d", amount, Core.Shared.Items[item].label, src))
            end
        else
            -- Notify and debug if player can't carry the item
            TriggerClientEvent('QBCore:Notify', src, 'You cannot carry more of this item!', 'error')
            if Config.Debug then
                print(string.format("^5Debug^7: Player %d cannot carry %d x '%s'", src, amount, Core.Shared.Items[item].label))
            end
        end
    end
end)
...
function HasItem(src, item, count)
    -- Returns true if the player has at least 'count' of the 'item'
    return exports.ox_inventory:Search(src, 'count', item) >= (count or 1)
end

function dupeWarn(src, item)
    -- Logs the potential duplication attempt and handles the exploit
    print(string.format("Potential duplication attempt by Player %d with item %s", src, item))
    DropPlayer(src, "Exploiting duplication bugs is prohibited.")
end
