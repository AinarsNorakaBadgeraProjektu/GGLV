		...
		if loc.Enable then
			...
			Targets[nameEnter] =
				exports['qb-target']:AddBoxZone(nameEnter, jobLoc.Enter.coords.xyz, jobLoc.Enter.w, jobLoc.Enter.d, { name=nameEnter, heading = jobLoc.Enter.coords.w, debugPoly=Config.Debug, minZ=jobLoc.Enter.coords.z-1, maxZ=jobLoc.Enter.coords.z+2 },
					{ options = { { event = "GGLV-Recycle:TeleWareHouse", icon = "fas fa-recycle", label = Loc[Config.Lan].target["enter"]..(Config.PayAtDoor and " ($"..Config.PayAtDoor..")" or ""), tele = jobLoc.Enter.tele, job = loc.Job, enter = true }, },
					distance = 1.5 })
			...
			Targets[nameExit] =
				exports['qb-target']:AddBoxZone(nameExit, jobLoc.Exit.coords.xyz, jobLoc.Exit.w, jobLoc.Exit.d, { name=nameExit, heading = jobLoc.Exit.coords.w, debugPoly=Config.Debug, minZ=jobLoc.Exit.coords.z-1, maxZ=jobLoc.Exit.coords.z+2, },
					{ options = { { event = "GGLV-Recycle:TeleWareHouse", icon = "fas fa-recycle", label = Loc[Config.Lan].target["exit"], tele = jobLoc.Exit.tele }, },
					distance = 1.5 })
			...
			Targets[nameDuty] =
				exports['qb-target']:AddBoxZone(nameDuty, jobLoc.Duty.coords.xyz, jobLoc.Duty.w, jobLoc.Duty.d, { name=nameDuty, heading = jobLoc.Duty.coords.w, debugPoly=Config.Debug, minZ=jobLoc.Duty.coords.z-0.5, maxZ=jobLoc.Duty.coords.z+0.5, },
					{ options = { { event = "GGLV-Recycle:dutytoggle", icon = "fas fa-hard-hat", label = Loc[Config.Lan].target["duty"], job = loc.Job, Trolly = jobLoc.Trolly }, },
					distance = 1.5 })
				...
				for i = 1, #jobLoc.Trade do
					...
					Targets[nameTrade] =
						exports['qb-target']:AddBoxZone(nameTrade, vec3(jobLoc.Trade[i].coords.x, jobLoc.Trade[i].coords.y, jobLoc.Trade[i].coords.z-1), jobLoc.Trade[i].w, jobLoc.Trade[i].d, { name=nameTrade, heading = jobLoc.Trade[i].coords.w, debugPoly=Config.Debug, minZ= jobLoc.Trade[i].coords.z-1, maxZ=jobLoc.Trade[i].coords.z+1 },
							{ options = { { event = "GGLV-Recycle:Trade:Menu", icon = "fas fa-box", label = Loc[Config.Lan].target["trade"], job = loc.Job, Ped = Peds[nameTrade] }, },
							distance = 1.5 })
				end
			end
		end
	...
	for i = 1, #Config.Locations["BottleBanks"] do local loc = Config.Locations["BottleBanks"][i]
		...
		Targets[nameBank] =
			exports['qb-target']:AddBoxZone(nameBank, vec3(loc.coords.x, loc.coords.y, loc.coords.z-1), 1.0, 1.0, { name=nameBank, heading = loc.coords.w, debugPoly = Config.Debug, minZ = loc.coords.z-1, maxZ=loc.coords.z+1 },
				{ options = { { event = "GGLV-Recycle:Bottle:Menu", icon = "fas fa-certificate", label = Loc[Config.Lan].target["sell_bottles"], job = Config.JobRole, Ped = Peds[nameBank] }, },
				distance = 1.5 })
	end
end)
...
function PickRandomPackage(Trolly)
	...
	Targets["Package"] =
		exports['qb-target']:AddTargetEntity(randPackage,
			{ options = { { event = "GGLV-Recycle:PickupPackage:Start", icon = 'fas fa-magnifying-glass', label = Loc[Config.Lan].target["search"], Trolly = Trolly} },
			distance = 2.5,	})
end

--Event to enter and exit warehouse
RegisterNetEvent("GGLV-Recycle:TeleWareHouse", function(data) local Ped = PlayerPedId()
	if data.enter then
				...
				if Config.PayAtDoor then
					if Config.Inv == "ox" then
						...
						if Config.Inv == "ox" then
							...
							local p = promise.new()	Core.Functions.TriggerCallback("GGLV-Recycle:GetCash", function(cb) p:resolve(cb) end)
							cash = Citizen.Await(p)
						end
						if cash >= Config.PayAtDoor then TriggerServerEvent("GGLV-Recycle:DoorCharge")
						else triggerNotify(nil, Loc[Config.Lan].error["no_money"], "error") return end
					end
				end
			else
		...
		DoScreenFadeOut(500)
		while not IsScreenFadedOut() do Wait(10) end
		if onDuty then TriggerEvent('GGLV-Recycle:dutytoggle') end
		SetEntityCoords(Ped, data.tele.xyz)
		DoScreenFadeIn(500)
	end
end)

RegisterNetEvent("GGLV-Recycle:PickupPackage:Start", function(data) local Ped = PlayerPedId()
	...
	if progressBar({label = Loc[Config.Lan].progressbar["search"], time = 5000, cancel = true, icon = "fas fa-magnifying-glass"}) then
		...
		TriggerEvent("GGLV-Recycle:PickupPackage:Hold", data)
	end
end)

RegisterNetEvent("GGLV-Recycle:PickupPackage:Hold", function(data) local Ped = PlayerPedId()
	...
	Targets["DropOff"] =
		exports['qb-target']:AddTargetEntity(TrollyProp,
			{ options = { { event = "GGLV-Recycle:PickupPackage:Finish", icon = 'fas fa-recycle', label = Loc[Config.Lan].target["drop_off"], Trolly = data.Trolly } },
			distance = 2.5,	})
end)

RegisterNetEvent("GGLV-Recycle:PickupPackage:Finish", function(data) local Ped = PlayerPedId()
...
RegisterNetEvent('GGLV-Recycle:dutytoggle', function(data)
...
local Selling = false
RegisterNetEvent('GGLV-Recycle:SellAnim', function(data) local Ped = PlayerPedId()
	...
	for k in pairs(Config.Prices) do
		if k == data.item then TriggerServerEvent('GGLV-Recycle:Selling:Mat', {item = data.item, Ped = data.Ped }) Selling = false lockInv(false) return end
	end
	TriggerServerEvent("GGLV-Recycle:TradeItems", { item = data.item, amount = data.amount })
	Selling = false
	lockInv(false)
end)

RegisterNetEvent('GGLV-Recycle:Selling:Menu', function(data)
	...
	if Config.Menu == "qb" then
		...
		sellMenu[#sellMenu+1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "GGLV-Recycle:CloseMenu" } }
	end
	for item, price in pairsByKeys(Config.Prices) do
		sellMenu[#sellMenu+1] = {
			...
			params = { event = "GGLV-Recycle:SellAnim", args = { Ped = data.Ped, item = item } },
			title = Core.Shared.Items[item].label, description = Loc[Config.Lan].menu["sell_all"]..price..Loc[Config.Lan].menu["each"],
			event = "GGLV-Recycle:SellAnim", args = { Ped = data.Ped, item = item },
		}
	end
...
--Recyclable Trader
RegisterNetEvent('GGLV-Recycle:Trade:Menu', function(data)
	if Selling then return end
	local tradeMenu = {}
	...
	if Config.Menu == "qb" then
		...
		tradeMenu[#tradeMenu+1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "GGLV-Recycle:CloseMenu" } }
	end
	local tradeTable = {}
	...
	for _, v in pairs(Config.RecycleAmounts["Trade"]) do
		tradeMenu[#tradeMenu+1] = {
			...
			params = { event = "GGLV-Recycle:SellAnim", args = { item = "recyclablematerial", amount = v.amount, Ped = data.Ped } },
			event = "GGLV-Recycle:SellAnim", args = { item = "recyclablematerial", amount = v.amount, Ped = data.Ped }
		}
		Wait(0)
	end
...
--Recyclable Trader
RegisterNetEvent('GGLV-Recycle:Bottle:Menu', function(data)
	if Selling then return end
	local tradeMenu = {}
	if Config.Menu == "qb" then
