...
bridge.removePlayerMoney = function(src, amount)
    exports.qbx_core:RemoveMoney(src, 'cash', amount, 'Item bought at hunting store')
end

bridge.addPlayerMoney = function(src, amount)
    exports.qbx_core:AddMoney(src, 'cash', amount, 'Hunting sell item')
end
...
bridge.giveOfflinePlayerMoney = function(identifier, amount)
    ...
    if player then
        exports.qbx_core:AddMoney(player.PlayerData.source, 'bank', amount, "Hunting tournament prize")
        return
    end
