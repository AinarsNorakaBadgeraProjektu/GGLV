...
RegisterNetEvent('evidence:client:SetStatus', function(statusId, time)
    if time > 0 and StatusList[statusId] then
        if (CurrentStatusList == nil or CurrentStatusList[statusId] == nil) or
            ...
            PolNotify.Send(Lang:t("notifyheaders.evidence"), CurrentStatusList[statusId].text, "error", 4500)
        end
    ...
    ProgressBar({
        ...
        if cb then
            if Blooddrops and next(Blooddrops) then
                ...
                PolNotify.Send(Lang:t("notifyheaders.evidence"), Lang:t("success.blood_clear"), "success", 4500)
            end
        else
            PolNotify.Send(Lang:t("notifyheaders.evidence"), Lang:t("error.blood_not_cleared"), "error", 4500)
        end
    end)
end)
    ...
    ProgressBar({
        ...
        if cb then
            if Casings and next(Casings) then
                ...
                PolNotify.Send(Lang:t("notifyheaders.evidence"), Lang:t("success.bullet_casing_removed"), "success", 4500)
            end
        else
            PolNotify.Send(Lang:t("notifyheaders.evidence"), Lang:t("error.bullet_casing_not_removed"), "error", 4500)
        end
    end)
end)

RegisterNetEvent('evidence:client:writeEvidenceNot', function(item)
    if Config.InputSystem == 'ox_lib' then textfield = 'input' else textfield = 'text' end
    local Options = {}
    Options[#Options+1] = {label = 'Note', name = 'evidencebagnote', type = textfield, required = true}
    local dialog = ContextSystem.Input('Write evidence note', Lang:t("menu.write"), Options)
    if dialog then
        if Config.InputSystem == 'ox_lib' then note = dialog[1] else note = dialog.evidencebagnote end
        if note and note ~= '' then
            TriggerServerEvent('police:server:setEvidenceBagNote', item, note)
        end
    else return end
end)
    ...
    if not WhitelistedWeapon(weapon) then
        ...
        if shotAmount > 5 and (CurrentStatusList == nil or CurrentStatusList['gunpowder'] == nil) then
            if math.random(1, 10) <= 7 then
                TriggerEvent('evidence:client:SetStatus', 'gunpowder', Config.GSRTime)
            end
        end
...
RegisterNetEvent('police:client:SelectEvidence', function(Data)
    QBCore.Functions.TriggerCallback('police:server:GetEvidenceByType', function(List)
        if List == nil then
            PolNotify.Send(Lang:t("notifyheaders.evidence"), Lang:t('error.dont_have_evidence_bag'), "error", 4500)
        else
...
RegisterNetEvent('police:client:ExamineEvidenceBag', function(Data)
    ProgressBar({
        time = 5000,
        label = Lang:t('progressbar.examining', {label = Data.label}),
        canCancel = true,
        disableControl = true
    }, function(cb)
        if cb then
...
RegisterNetEvent('hd-policejob:client:StartGSRTest', function()
    if not PlayerJob.type == 'leo' and not PlayerJob.onduty then return end
    local player, distance = Framework.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("hd-policejob:server:StartGSRTest", playerId)
    else
        PolNotify.Send(Lang:t("notifyheaders.general"), Lang:t("error.none_nearby"), "error", 4500)
    end
end)

RegisterNetEvent('hd-policejob:evidence:PlayGSRAnimation', function(type, coords, playerId)
    TaskTurnPedToFaceCoord(PlayerPedId(), coords.x, coords.y, coords.z, 1000)
    Wait(1000)
    CreateThread(function()
         ProgressBar({
            time = 8000,
            label = Lang:t('progressbar.takinggsr'),
            canCancel = true,
            disableControl = true
        }, function(cb) 
            if cb then
                QBCore.Functions.TriggerCallback('police:GetGSRStatus',function(gsrpositive)
                    if gsrpositive then
                        PolNotify.Send(Lang:t("notifyheaders.evidence"), 'GSR Positive', "error", 4500)
                    else
                        PolNotify.Send(Lang:t("notifyheaders.evidence"), 'GSR Negative', "success", 4500)
                    end
                end,playerId)
            end
        end)
    ...
    if type == 'take' then
        local Anim = { AnimDict = 'amb@prop_human_parking_meter@male@enter', AnimName = 'enter', Duration = 1370, BlendInSpeed = 5.0, BlendOutSpeed = 5.0, Flag = 0 }
        PolFunctions.PlayAnim(Anim)
        Wait(1370)
        local Anim2 = { AnimDict = 'amb@prop_human_parking_meter@male@base', AnimName = 'base', Duration = 2130, BlendInSpeed = 5.0, BlendOutSpeed = 5.0, Flag = 0 }
        PolFunctions.PlayAnim(Anim2)
        Wait(2130)
        local Anim3 = { AnimDict = 'amb@prop_human_parking_meter@male@exit', AnimName = 'exit', Duration = 1330, BlendInSpeed = 5.0, BlendOutSpeed = 5.0, Flag = 0 }
        PolFunctions.PlayAnim(Anim3)
        Wait(1330)
    elseif type == 'give' then
        local Anim = { AnimDict = 'gestures@m@car@van@casual@ps', AnimName = 'gesture_hand_left_three', Duration = 2830, BlendInSpeed = 5.0, BlendOutSpeed = 5.0, Flag = 0 }
        PolFunctions.PlayAnim(Anim)
        Wait(2830)
        local Anim2 = { AnimDict = 'gestures@m@car@van@casual@ps', AnimName = 'gesture_hand_right_three', Duration = 2830, BlendInSpeed = 5.0, BlendOutSpeed = 5.0, Flag = 0 }
        PolFunctions.PlayAnim(Anim2)
        Wait(2830)
    end
end)
