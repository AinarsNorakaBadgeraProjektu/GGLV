...
local function BuildHelicopterTable()
    helicopterModels = {}
    for _, garageData in pairs(Config.Garage) do
        if garageData.list then
            for _, vehicleData in pairs(garageData.list) do
                if vehicleData.category == "helicopter" then
                    helicopterModels[vehicleData.model] = true
                end
            end
        end
    end
...
RegisterNetEvent('police:client:VehicleInfo', function(data)
    if data.event == 'Entered' then
        if helicopterModels[string.lower(data.name)] then
            inHeli = true
            return
        end
