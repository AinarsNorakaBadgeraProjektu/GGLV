...
if Config.Framework == 'qb' then
    ...
    RegisterNetEvent('osp_ambulance:stash', function()
        ...
        if onDuty then
            if not BodyDamage.stashSetup then
                BodyDamage.stashSetup = true
                if Config.UsedInventory == 'qs' then
                    exports['qs-inventory']:RegisterStash("ambulancestash_" .. GetPlayerData().citizenid, 20, 20000)
                elseif Config.UsedInventory == 'ox' or Config.UsedInventory == 'origen' then
            ...
            if Config.UsedInventory == 'qs' then
                ...
                other.slots = 50
                TriggerServerEvent("inventory:server:OpenInventory", 'Ambulance Stash',
                    "ambulancestash_" .. GetPlayerData().citizenid, other)
            ...
            elseif Config.UsedInventory == 'chezza' then
                TriggerEvent('inventory:openStorage', "Locker", "ambulancestash_" .. GetPlayerData().citizenid, 20, 2000,
                    { "ambulance" })
            else
    ...
    RegisterNetEvent('osp_ambulance:stash', function()
        ...
        if onDuty then
            ...
            if Config.UsedInventory == 'qs' then
                ...
                other.slots = 50
                TriggerServerEvent("inventory:server:OpenInventory", 'Ambulance Stash',
                    "ambulancestash_" .. GetPlayerData().identifier, other)
            ...
            elseif Config.UsedInventory == 'chezza' then
                TriggerEvent('inventory:openStorage', "Locker", "ambulancestash_" .. GetPlayerData().identifier, 20, 2000,
                    { "ambulance" })
            else
...
function CreateBloodDropEvidence()
    ...
    if Config.Framework == 'qb' then
        if GetPlayerData().metadata then
            TriggerServerEvent("evidence:server:CreateBloodDrop", GetPlayerData().citizenid,
                GetPlayerData().metadata["bloodtype"], coords)
        end
    end
end
        ...
        while true do
            ...
            if performingcpr then
                if not IsEntityPlayingAnim(PlayerPedId(), 'mini@cpr@char_a@cpr_str', 'cpr_pumpchest', 3) then
                    TaskPlayAnim(PlayerPedId(), "mini@cpr@char_a@cpr_str", "cpr_pumpchest", 8.0, -8.0, -1, 1, 0, false,
                        false, false)
                end
...
function DeadMovement(ped)
    ...
    if IsDisabledControlJustPressed(0, 32) then
        -- Wait(1000)
        ClearPedTasks(ped)
        TaskPlayAnimAdvanced(ped, "move_injured_ground", "front_loop", GetEntityCoords(ped), 1.0, 0.0,
...
function dispatchNotify()
    ...
    TriggerServerEvent('qs-dispatch:server:CreateDispatchCall', {
        ...
        callCode = { code = 'INJURED CITIZEN', snippet = 'EMS' },
        message = 'A ' .. playerData.sex .. ' is injured badly',
        flashes = true,
        image = nil,
        blip = {
            sprite = 61,
...
RegisterNetEvent("osp_ambulance:appearanceEnter",
    function()                                               -- Disable wounding when appearance is open (have been reported to be causing issues with the wound system)
...
function initLanguages() -- Initilizes the ui language
    if lang == nil then
        while true do
            Wait(100)
            print('FATAL ERROR! osp_ambulance/lang.lua does not work properly!')
        end
    end
    SendNUIMessage({
        type = "lang",
    ...
    if Config.Framework == 'qb' then
        SetTimeout(math.random(2500, 4000), function()
            ...
            TriggerServerEvent('qb-phone:server:sendNewMail', {
                ...
                { gender = gender, lastname = charinfo.lastname, costs = amount },
                button = {}
            })
        end)
    else
