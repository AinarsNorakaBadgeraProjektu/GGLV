...
local function InitiateRegisterRobbery()
    local canStart = lib.callback.await('GGLV-StoreRobbery:StartRobbery', false)
    ...
    if not skillcheck then
        TriggerServerEvent('GGLV-StoreRobbery:DoesLockpickBreak')
    ...
    if ProgressBar(Config.Animations.register) then
        ...
        if codeChance <= Config.Registers.noteChance then
            ...
            if note == 'confirm' then
                activeSafe = true
                TriggerServerEvent('GGLV-StoreRobbery:CompleteRegisterRobbery')
            end
        else
            activeComputer = true
            TriggerServerEvent('GGLV-StoreRobbery:CompleteRegisterRobbery')
...
local function InitiateComputerHack()
    ...
    if failedHack > Config.Computers.maxAttempts then
        ...
        TriggerServerEvent('GGLV-StoreRobbery:FailedRobbery')
        return
    end
...
local function InitiateSafeRobbery()
    ...
    elseif convertedCode == safePin then
        ...
        if ProgressBar(Config.Animations.safe) then
            activeRegister = false
            TriggerServerEvent('GGLV-StoreRobbery:CompleteSafeRobbery')
            safePin = nil
        else
            activeSafe = true
        end
...
RegisterNetEvent('GGLV-StoreRobbery:client:showBoostingDialog', function()
    local alert = lib.alertDialog({
        header = 'Noslēpumaina informācija',
        content = 'Ar strādājošu klēpjdatoru vari iesaistīties melnajā tirgū, izmantojot kripto naudu un veikt speciālas auto zādzības, kuras iedalās klasēs.',
        centered = true,
        cancel = true
    })
end)