    ...
    window.addEventListener('message', function(event) {
        ...
        switch (data.action) {
            ...
            case 'addPhoneConversation':
                ...
                createMessageWithJQuery(data.npcID, data.itemName, data.placeIndex);
                break;
            ...
            case 'cancelConversation':
                conversations[data.conversationID].cancelConversation = true;

                addMessageToConversation(data.conversationID, "Jednak nie mam tyle czasu, żegnaj.", false);
                break;
            case 'showModal':
...
function loadPlayerExperience(data) {
    ...
    setSalesProgress(data.skill, data.skillLimit)
}
...
$('#progress-sales').mouseenter(function(e) {
    updateTooltip( translate("dialog_sales_skill_tooltip", playerExperience.skill, playerExperience.skillLimit), e);
}).mousemove(function(e) {
    updateTooltip( translate("dialog_sales_skill_tooltip", playerExperience.skill, playerExperience.skillLimit), e); // Aktualizacja pozycji przy ruchu myszki
}).mouseleave(function() {
    hideTooltip();
});
    ...
    for (var key in inventory) {
        if (inventory.hasOwnProperty(key)) {
            ...
            if (item.count > 0) {
                ...
                itemDiv.append($("<p>").text(item.count));
        
                // Adding click handling for the inventory item
                itemDiv.click((function(key, item) {
                    return function() {
                        selectInventoryItem(key, item.label, item.count);
                    };
                })(key, item));
        
                itemName.mouseenter((function(item) {
                    return function(e) {
                        updateTooltip(item.label, e, true);
                    };
                })(item)).mousemove((function(item) {
                    return function(e) {
                        updateTooltip(item.label, e, true); // Update position on mouse move
                    };
                ...
                var confirmIcon = $("<img>", {
                    ...
                    click: function() {
                        ...
                        if (!isNaN(numericValue) && numericValue.toString() === inputValue && numericValue >= 1) {
                            $.post('https://is_selldrugs/dialogBack', JSON.stringify({ action: "sell", ped: ped, itemName: selectedItemName, inputValue: numericValue, count: npcQuantity[itemToSellName], isWholesale: false }))
                                .done(function(response) {
                                    if (response.result == "cancel") {
                                        loadDialog("sellFail");
                                    } else if (response.result == "negotiation") { 
                                        loadDialog("giveMeBetterOption");
                                    } else if (response.result == "success") { 
                                        itemSellData = {
                                            itemName: selectedItemName, 
                                            inputPrice: numericValue, 
                                            count: npcQuantity[itemToSellName]
                                        }
        ...
        if (!isTyping) {
            ...
            if (response.action !== null) {
                ...
                if (response.endInteraction !== null && response.endInteraction) {
                    $.post('https://is_selldrugs/successInteraction', JSON.stringify(itemSellData))
                } else {
                    $.post('https://is_selldrugs/dialogBack', JSON.stringify({ action: response.action }))
                }
