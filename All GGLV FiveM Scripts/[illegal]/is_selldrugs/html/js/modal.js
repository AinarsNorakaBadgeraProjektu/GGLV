...
function handleUserChoice(choice) {
    $('#modal').fadeOut();
    $.post('https://is_selldrugs/modalChoice', JSON.stringify(choice));
}
...
function showDeleteConfirmationModal() {
    ...
    $('#modal-button-accept').off('click').on('click', function() {
        if (currentDisplayedConversationId !== undefined) {
            $.post('https://is_selldrugs/playerDeletedConversation', JSON.stringify(currentDisplayedConversationId));
