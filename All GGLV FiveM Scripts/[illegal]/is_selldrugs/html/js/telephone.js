...
function togglePhoneUI(show) {
    if (show) {
        ...
        $.post('https://is_selldrugs/phoneExit');
    }
}
    ...
    callAudio.onended = function() {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                $.post('https://is_selldrugs/phoneStartTradeMission', JSON.stringify({ conversationId: conversationId, quantity: quantity }) )
...
var conversations = {};
function createMessageWithJQuery(messageId, itemName, placeIndex) {
    ...
    conversation.messages.forEach(function(message, index) {
        ...
        if (index == conversation.messages.length - 1 && conversation.gps) {
            ...
            $image.click(function() {
                ...
                $.post('https://is_selldrugs/setWaypoint', JSON.stringify({ coords: gpsCoords }))
            });
        }
...
function inputAccept(type, inputField, conversationId, conversation) {
    ...
    if (!isNaN(numericValue) && numericValue.toString() === inputValue && numericValue >= 1) {
        if (type == 'amount') {
            ...
            var randomDelay = Math.floor(Math.random() * (5500 - 3000 + 1)) + 3000;
            $.post('https://is_selldrugs/wholesaleNegotiatePrice', JSON.stringify({ conversationId: conversationId, inputValue: inputNumber }))
                .done(function(data) {
...
$('#notificationSwitch').change(function() {
    if (canChangeSwitch(lastNotificationSwitchTime)) {
        ...
        $.post('https://is_selldrugs/saveSettings', JSON.stringify({ statusAlerts: statusAlerts, statusAlertsSound: statusAlertsSound }));
    } else {
