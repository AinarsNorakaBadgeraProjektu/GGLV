...
function ChangeClothes(type)
    ...
    if type == "work" then
        if GetEntityModel(PlayerPedId()) == 1885233650 then
            ...
            SetPedPropIndex(PlayerPed, 0, Config.Clothes.femaleHelmet.clotheId, Config.Clothes.femaleHelmet.variation,
                true)
        end
    else
        if Config.Framework == "QBCore" then
...
for k, v in pairs(Config.Clothes.male) do
    table.insert(Config.realClothes.male,
        { component_id = componentIdTranslation[k], drawable = v.clotheId, texture = v.variation })
end
