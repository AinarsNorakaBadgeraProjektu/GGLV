...
Config.RequiredJob =
"builderjob"                           -- Set to "none" if you dont want using jobs. If you are using target, you have to set "job" parameter inside every export in target.lua
...
Config.OnePercentWorth = 15            -- Means that 1% progress will reward players with $100
...
Config.JobCooldown = 0 *
60                                        -- 10 * 60            -- 0 minutes cooldown between making jobs (in brackets there's example for 10 minutes)
...
Config.RewardItemsToGive = {
    ...
    {
        item_name = "WEAPON_FURY_PICKAXE_IRON",
        chance = 1,
...
Config.RequiredItem =
"none"                                   -- Set it to anything you want, to require players to have some item in their inventory before they start the job
...
Config.Lang = {
    ...
    ["tutorialAfterMixerPickup"] =
    "You just picked up your boss's designated concrete mixer. Drive it to the construction site, then pour the concrete in the designated areas. The Construction Site is marked on the GPS",
    ["startingTutorial"] =
    "Welcome to the work of the builder. A variety of tasks await you on site, from welding, to building walls, making drains, or pouring concrete. If you're working in a group, you'll now have to split up, one person have to be dropped off at the concrete mixer pick-up point and then transport the concrete mixer to the construction site. Every single activity, will be explained when you first approach it. You have the locations of the tasks marked on the map. If you work alone, then you have to complete each task by yourself, if you work in groups - you decide how you want to divide. You don't have to do all the tasks, you can finish when you want, and your payment depends on how many % of the tasks you do. We wish you good luck!",
    ["installBlock"] = "Build a wall",
    ["tutorialWallBuilding"] =
    "You are next to the wall frame, your task is to build it using concrete blocks. The concrete blocks are near the frame, and are marked on the GPS. Walk up to the pile with the blocks, pick one up, and then mount it to the wall",
    ["tutorialAboutPipes"] =
    "You are next to a hole overlooking sewer pipes. Your task is to install the pipes in the right places. After installing the pipes, you must pour concrete over the hole using a concrete mixer. ",
    ["tutorialWelding"] =
    "You are next to the place where the boss told you to weld something. The job is very simple, requiring no further explanation",
