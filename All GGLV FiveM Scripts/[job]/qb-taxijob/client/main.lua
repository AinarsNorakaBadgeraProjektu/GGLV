                ...
                if dist < 25 then
                    ...
                    if dist < 5 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            TriggerServerEvent('qb-taxi:server:NpcPay', NpcData.startingLength, NpcData.distanceLeft, NpcData.CrashCount == 0)
        ...
        while NpcData.NpcTaken do
            ...
            if isInsideDropZone then
                if IsControlJustPressed(0, 38) then
                    ...
                    if NpcData.startingLength == 0 or NpcData.distanceLeft == 0 or NpcData.startingLength < NpcData.distanceLeft then
                        QBCore.Functions.Notify('Nauda netika izmaksāta, jo konstatēts mēģinājums apkrāpt sistēmu!', 'error', 5000)
                        TriggerEvent('qb-taxi:client:CancelTaxiNpc')
                        return
                    end
                    TriggerServerEvent('qb-taxi:server:NpcPay', NpcData.startingLength, NpcData.distanceLeft, NpcData.CrashCount == 0)
...
CreateThread(function()
    while NpcData.Active do
        Wait(1000)
        NpcData.CountDown = NpcData.CountDown - 1
        if NpcData.CountDown <= 0 then
            TriggerEvent('qb-taxi:client:CancelTaxiNpc')
            QBCore.Functions.Notify('Uzdevums neizpildīts laikā!', 'error')
            break
        end
    end
end)