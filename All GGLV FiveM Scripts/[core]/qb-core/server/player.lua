...
function QBCore.Player.GetTotalWeight(source)
    if GetResourceState('ox_inventory') == 'missing' then return end

    local totalWeight = 0
    local inventory = exports['ox_inventory']:Search(source, 'all')
    for _, item in pairs(inventory) do
        local itemWeight = QBCore.Shared.Items[item.name].weight or 0
...
function QBCore.Player.GetSlotsByItem(source, itemName)
    if GetResourceState('ox_inventory') == 'missing' then return end
...
function QBCore.Player.GetFirstSlotByItem(source, itemName)
    if GetResourceState('ox_inventory') == 'missing' then return end
