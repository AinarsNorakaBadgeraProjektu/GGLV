...
local lastPaycheckHour = nil

function PaycheckInterval()
    local currentTime = os.date("*t")
    if currentTime.min == 0 and lastPaycheckHour ~= currentTime.hour then
        lastPaycheckHour = currentTime.hour

        if QBCore.Players and next(QBCore.Players) then
            for _, Player in pairs(QBCore.Players) do
                if Player then
                    local jobName = Player.PlayerData.job.name
                    local jobGrade = tostring(Player.PlayerData.job.grade.level)
                    local jobInfo = QBShared.Jobs[jobName]
                    if jobInfo and jobInfo.grades[jobGrade] then
                        local payment = jobInfo.grades[jobGrade].payment or Player.PlayerData.job.payment
                        if payment > 0 and (jobInfo.offDutyPay or Player.PlayerData.job.onduty) then
                            local accountBalance = exports['qb-banking']:GetAccountBalance(jobName)
                            if accountBalance >= payment then
                                exports['qb-banking']:RemoveMoney(jobName, payment, 'Employee Paycheck')

                                if GetResourceState('vms_cityhall') == "started" then
                                    exports['vms_cityhall']:updatePaychecks(Player.PlayerData.source, payment)
                                end

                                TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, "You have received your paycheck!", "success")
                            else
                                TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, "Your company does not have enough money to pay you.", "error")
                            end
                        end
                    end
                end
    ...
    SetTimeout(30 * 1000, PaycheckInterval)
end
